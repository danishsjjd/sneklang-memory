...
void sweep(vm_t* vm) {
  ...
  for (size_t i = 0; i < vm->objects->count; i++) {
    ...
    if (obj->is_marked) {
      ...
      // Unreachable: free it and null out its slot
...
void vm_collect_garbage(vm_t* vm) {
  ...
  mark(vm);   // Step 1: mark roots from frames
  trace(vm);  // Step 2: follow references
  sweep(vm);  // Step 3: free unmarked objects
}
...
void trace(vm_t* vm) {
  stack_mt* gray_objects = stack_new(8);
...
void trace_blacken_object(stack_mt* gray_objects, snek_object_t* ref) {
...
void trace_mark_object(stack_mt* gray_objects, snek_object_t* obj) {
